...
function createOptimizedChunks(course) {
  ...
  // 4. Module chunks - only first 2 modules, condensed
  const modulesToProcess = course.modules.slice(0, 2);
  modulesToProcess.forEach((module, index) => {
    ...
    chunks.push({
      ...
      metadata: {
        ...
        module_index: index,
        topics: module.topics ? module.topics.slice(0, 3) : []
      }
    });
  });
...
console.log('   ✓ Only 2 modules per course instead of all');
console.log('   ✓ Condensed highlights and features');
console.log('   ✓ Consistent embeddings for better caching');
console.log('');
console.log('🔥 Expected token savings: 60-70% reduction in context size');
