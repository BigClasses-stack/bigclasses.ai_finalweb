...
export function useGeminiChat(): UseGeminiChatReturn {
  ...
  useEffect(() => {
    const initChat = () => {
      ...
      // Always create a brand new chat service
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      const errorMessage: Message = {
        ...
        type: 'bot',
        content:
          "I seem to be having some technical difficulties right now. Please try again in a moment, or contact our support team directly.",
        timestamp: new Date(),
      };
