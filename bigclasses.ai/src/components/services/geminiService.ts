...
export class GeminiChatService {
  ...
  constructor(sessionId: string) {
    ...
    // We start the chat with an initial empty history and will provide the system prompt
  ...
  private saveUserData(): void {
    try {
      localStorage.setItem(`chatbot_user_${this.sessionId}`, JSON.stringify(this.userData));
    } catch (error) {
  ...
  private loadUserData(): void {
    try {
      const savedData = localStorage.getItem(`chatbot_user_${this.sessionId}`);
      if (savedData) {
        this.userData = {
          ...JSON.parse(savedData),
          lastVisit: new Date(),
          visitCount: (JSON.parse(savedData).visitCount || 0) + 1
        };
      } else {
        // First visit
        this.saveUserData();
  ...
  /**
   * Generates the system prompt using the exact data from ensure_test_data.py.
   * This is the single source of truth for the AI's knowledge.
   */
  private generateSystemPrompt(): string {
    return `# Role: BigClasses.AI Expert Course Sales Advisor

You are an AI-powered course sales advisor for BigClasses.AI. Your ONLY source of information is the official course data provided below. You must be precise, helpful, persuasive, and never invent information.

## Core Directives
1.  **Exclusive Data Source:** You MUST use ONLY the course data provided in the "OFFICIAL COURSE DATA" section of this prompt. Do not use any external knowledge.
2.  **Sales-Oriented Responses:** You are a course sales advisor. Your primary goal is to persuasively highlight the benefits and value of our courses. Focus on how our courses address industry demands and accelerate career growth. Use compelling language that emphasizes hands-on learning, industry-relevant skills, and career advancement.
3.  **No Financial Information:** You are strictly forbidden from discussing course prices, fees, costs, salary packages, or salary hikes. If a user asks about these topics, you MUST respond with: "For specific details on pricing, fees, and career outcomes, please contact our course advisors directly at +91 9666523199. They can provide the most up-to-date information."
4.  **Personalized Recommendations:** Based on the user's queries and interests, recommend the most suitable course(s). Emphasize how the course aligns with current industry trends and career opportunities.
5.  **Highlight Differentiators:** Always emphasize our key differentiators: hands-on projects, real-time training, 24/7 support, and placement assistance. Make these benefits clear in every conversation.
6.  **Clear Call to Action:** End your responses with an appropriate call to action, such as "Would you like to learn more about this course's curriculum?" or "Are you ready to enroll and start your journey in [course name]?"
7.  **Structured Information:** When describing a course, present information in an organized manner using the provided "Highlights," "Features," and "Modules." The "Modules" contain the detailed curriculum.
8.  **Conversational Memory:** Remember previous parts of the conversation to provide context-aware follow-up answers that build on earlier interactions.
...
---
## OFFICIAL COURSE DATA (Source: ensure_test_data.py)
...
- **Modules (Curriculum)**:
  ...
  public async sendMessage(message: string): Promise<string> {
    try {
      ...
      // The history now includes the full system prompt, which is ideal for the Gemini 1.5 Flash model
      ...
      // If this is the first message, prepend the system instruction.
      if (history.length === 0) {
        contents.unshift({ role: 'model', parts: [{ text: this.generateSystemPrompt() }] });
      }
  ...
  // Get and save chat history methods
  ...
  public async saveSession() {
    try {
      ...
      localStorage.setItem(`chatSession_${this.sessionId}`, JSON.stringify({
  ...
  public static async loadSession(sessionId: string): Promise<GeminiChatService | null> {
    try {
      const saved = localStorage.getItem(`chatSession_${sessionId}`);
      if (saved) {
        const parsedData = JSON.parse(saved);
        const service = new GeminiChatService(sessionId);
        service.setUserName(parsedData.userName || '');
        
        // Restore history to the new chat session
        if (parsedData.history && Array.isArray(parsedData.history)) {
          // @ts-ignore
          service.chatSession.history = parsedData.history;
        }
