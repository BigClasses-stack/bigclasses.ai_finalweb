...
const getRelatedFeatures = (currentId: string) => {
  // Return all features except the current one
  return features.filter((feature: Feature) => feature.id !== currentId);
};
...
const FeatureDetail: React.FC = () => {
  ...
  const [currentFeatureData, setCurrentFeatureData] = useState<Feature | null>(null);
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      const featureData = features.find((feature: Feature) => feature.id === id) || features[0];
  ...
  // Fix navigation path (add quotes and backticks)
  const handleFeatureNavClick = (featureId: string) => {
  ...
  const getCustomSmallImage = (featureId: string) => {
  ...
  const getCustomDescription = (featureId: string) => {
              ...
              <div className="feature-content">
                ...
                <div className="related-features">
                  ...
                  <div className="related-cards">
                    {relatedFeatures.map((relatedFeature: Feature) => (
